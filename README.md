...
| /api/v1/apiServices/loginUser/ | email o username, password | POST | token, success, msg, err(l=invalid-login,g=login-google,f=login-facebook), user, timetoken | Login aplicativo mediante email o username y contraseña |
...
| /api/v1/apiServices/newNotice/ | kind(1,2), thing(P,S), title, category, time, state, offer, place, colors, description, locations, urgency, quantity | POST | success, msg, err, notice | Nueva publicacion de servicio o producto |
| /api/v1/apiServices/updateNotice/ | notice, title, category, time, state, offer, place, colors, description, locations, urgency, visibility, quantity | PUT | success, msg, err | Edicion publicacion de servicio o producto |
